...
from llava.mm_utils import tokenizer_image_token, create_pseudo_labels

from PIL import Image

from llava.llm_src import LlamaDecoderLayer, LlamaAttention
...
def get_peft_state_maybe_zero_3(named_params, bias):
    if bias == "none":
        to_return = {k: t for k, t in named_params if "lora_" in k and 'visual_processing' not in k}
    elif bias == "all":
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if data_path:  # Allow data_path to be None for ROITrainingDataset's init sequence
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if fair_lazy: sources['conversations'] = sources['conversations'][:2] #same amount of training samples in lazy version training for roi prediction
        if isinstance(i, int):
            sources = [sources]
        ...
        if 'image' in sources[0]:
            ...
            try:
                #print(f'Loading image {image_file} from {image_folder}...')
                image = Image.open(os.path.join(image_folder, image_file)).convert('RGB')
                src_images = image
            except OSError as e:
                print(str(e))
                if "truncated" in str(e):
                    print(f"Skipping truncated image: {os.path.join(image_folder, image_file)}")
                    #create an all white image with  shape of 336*336
                image = Image.new('RGB', (336, 336), color='white')
                src_images = image
                #see https://huggingface.co/datasets/howard-hou/OCR-VQA/discussions/2, in my case 1437717772.jpg
        ...
        data_dict['src_images'] = src_images
...
class ROITrainingDataset(LazySupervisedDataset):
    """Dataset for RoI prediction, inheriting from LLaVA's LazySupervisedDataset."""

    def __init__(self, roi_data_path: str,
                 tokenizer: transformers.PreTrainedTokenizer,
                 data_args: DataArguments,
                 roi_sink_threshold: float = 1e-3,
                 roi_binary_coeff: float = 0.2,
                 roi_samples = -1,
                 ):

        # Initialize parent with None for data_path, as we're loading and processing our own.
        super().__init__(data_path=None, tokenizer=tokenizer, data_args=data_args)
        ...
        self.list_data_dict = []  # This will be populated with LLaVA-compatible dicts
        ...
        for roi_sample in custom_list_data_dict:
            converted_sample = {
                ...
                "image": roi_sample['image'],  # This is a path
                "conversations": [
    ...
    def __getitem__(self, i) -> dict[str, torch.Tensor]:
        ...
        data_dict['roi_target_map'] = torch.tensor(roi_target_map)
        return data_dict
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if 'src_images' in instances[0]:
            src_images = [instance['src_images'] for instance in instances]
            batch['src_images'] = src_images
        return batch


def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
                                data_args, roi_super_type='lazy',
                                ) -> Dict:
    ...
    if roi_super_type == 'lazy':
        ...
        train_dataset = ROITrainingDataset(
            roi_data_path=data_args.data_path,
            tokenizer=tokenizer,
            data_args=data_args,
            roi_samples = data_args.roi_samples

        )
    else: raise NotImplementedError(f"Unsupported roi_super_type: {roi_super_type}. Supported types are 'lazy' and 'v1'.")
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            original_config = transformers.AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            ...
            config = config_cls.from_dict(config.to_dict())
    ...
    if model_args.vision_tower is not None:
        ...
        if model.enable_twig and training_args.fp16:
            rank0_print('using fp16 training for twig weights')
    ...
    if model_args.enable_twig:
        #freeze all parameters except the roi layers
        for param in model.parameters():
            param.requires_grad = False
        unfreeze_layers = model_args.twig_freeze if getattr(model_args, 'twig_freeze', None) is not None else 0
        for i, twig_layer_module in enumerate(model.model.twig_layers[unfreeze_layers:]):
            for param in twig_layer_module.parameters():
                param.requires_grad = True
        for param in model.model.twig_norm.parameters():
            param.requires_grad = True
        for param in model.twig_head.parameters():
            param.requires_grad = True

    data_module = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args, roi_super_type=model_args.roi_super_type)

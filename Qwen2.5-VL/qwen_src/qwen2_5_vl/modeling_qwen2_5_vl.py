...
from ..mm_utils import get_batched_sub_images, interplot_img_feat, insert_sub_feat, plot_tensor_2d, get_singleturn_query_text_hs_mheads, \
get_foreground_bbox_torch
import copy
from qwen_src.qwen2_vl.image_processing_qwen2_vl_fast import smart_resize
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    def __init__(self, config: Qwen2_5_VLConfig):
        ...
        self.generation_attention_mask = None  # Used in fused 2-stage generationion_mask
        if getattr(config, "enable_twig", False):
            ...
            for i in range(self.twig_T):
                ...
                new_twig_layer = Qwen2_5_VLDecoderLayer(config, layer_idx=self.twig_K + i)
                self.twig_layers.append(new_twig_layer)
            self.twig_norm = Qwen2RMSNorm(config.hidden_size, eps=config.rms_norm_eps)
            ...
            if self.roi_super_type == 'lazy':
                ...
                raise NotImplementedError(
                    "Lazy supervision is not implemented yet. Please use 'v1' for roi_super_type."
                )
    ...
    def forward(
        ...
        if self.training is False and seq_length == 1 and self.generation_attention_mask is not None:
            # If we are in a fused 2-stage generation, the mask passed by `generate` is stale.
            # We use our stored mask from the first pass and append a '1' for the current token.
            if self.generation_attention_mask.shape[1] == past_key_values.get_seq_length():
                attention_mask = torch.cat(
                    [self.generation_attention_mask, self.generation_attention_mask.new_ones((self.generation_attention_mask.shape[0], 1))],
                    dim=-1
                )
                # Update the stored mask for the next step
        ...
        if cache_position is not None and seq_length==1 and past_seen_tokens and self.enable_twig:
            ...
            # When using 2-stage generation, the attention_mask is dynamically changed in the first forward pass
            # and may contain padding. This line incorrectly overwrites the carefully constructed mask with all
            # ones, breaking attention for subsequent generation steps. We disable this overwrite when 2-stage is enabled.
        ...
        if self.enable_twig and image_token_start>0:
            self.generation_attention_mask = None
            self.last_pos_id = None
            #visual_token_num = (input_ids == self.config.image_token_id).sum().item()
            #visual_masks = (input_ids == self.config.image_token_id) | (input_ids == self.config.video_token_id)
            visual_token_num = (image_grid_thw.squeeze(0)[0] * image_grid_thw.squeeze(0)[1] * image_grid_thw.squeeze(0)[2]) // 4
            ...
            for twig_layer in twig_layers:
                ...
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        twig_layer.__call__, twig_hidden_states, causal_mask, position_ids,
                        past_key_values, output_attentions, use_cache,cache_position,position_embeddings)
                else:
                    layer_outputs = twig_layer(
                        twig_hidden_states, attention_mask=causal_mask, position_ids=position_ids,
            ...
            #### ROI branch to predict the ROI mask
            seq_length = hidden_states.shape[1]
            if self.roi_enable2stage and seq_length>1 and image_token_start>0 and image_grid_thw.shape[0]==1:
                for i in range(self.twig_K, len(self.layers)):
                ...
                q, k = self.twig_layers[-1].input_layernorm(twig_hidden_states), self.twig_layers[-1].input_layernorm(twig_hidden_states)
                ...
                if self.roi_multi_head == False:
                    ...
                    sample_bsz, q_len, _ = query_states.size()  # sample_bsz will be 1
                    k_len = key_states.size(1)
                    num_heads, head_dim = self.twig_layers[-1].self_attn.num_heads, self.twig_layers[-1].self_attn.head_dim
                    num_key_value_heads = self.twig_layers[-1].self_attn.num_key_value_heads
                    query_states = query_states.view(sample_bsz, q_len, num_heads, head_dim).transpose(1, 2)
                    key_states = key_states.view(sample_bsz, k_len, num_key_value_heads, head_dim).transpose(1, 2)
                    position_embeddings = self.rotary_emb(twig_hidden_states[0:0 + 1], position_ids[:, 0:0 + 1])
                    cos, sin = position_embeddings
                    query_states, key_states = apply_multimodal_rotary_pos_emb(
                        query_states, key_states, cos, sin, self.twig_layers[-1].self_attn.rope_scaling["mrope_section"]
                    )
                    key_states = repeat_kv(key_states, self.twig_layers[-1].self_attn.num_key_value_groups)
                    last_query_states = query_states[:,:,-1:,:]
                    key_states = key_states[:,:,image_token_start:image_token_start + visual_token_num]
                    mask_score = torch.matmul(last_query_states, key_states.transpose(2, 3)) / math.sqrt(head_dim)
                    if self.roi_loss == 'mse':
                        mask_score = nn.functional.softmax(mask_score, dim=-1, dtype=torch.float32).to(query_states.dtype)
                    pred_roi = mask_score.mean(dim=1)

                    if visual_token_num>1024:
                        question_queries = query_states[:,:,image_token_start+visual_token_num:,:]
                        quest2key_score = torch.matmul(question_queries, key_states.transpose(2, 3)) / math.sqrt(head_dim)
                        question2key_roi = quest2key_score.mean(dim=1)
                        visual_hidden_states = hidden_states[:, image_token_start:image_token_start + visual_token_num, :]
                        viusal_sink_hs = visual_hidden_states[:, :1]
                        sim2sink_map = torch.cosine_similarity(visual_hidden_states, viusal_sink_hs, dim=-1).reshape(-1, feat_h, feat_w)
                        question_roi = question2key_roi.max(1)[0].reshape(-1, feat_h, feat_w).sigmoid() #* (sim2sink_map<=0.64).float()
                        question_roi_smoothed = question_roi #torchvision_F.gaussian_blur(question_roi.unsqueeze(0), kernel_size=3, sigma=1.0).squeeze(0)
                        question_roi_binaries = question_roi_smoothed>0.01
                        pass
                pred_roi = pred_roi.reshape(-1, feat_h, feat_w)  # reshape to (batch_size, feat_h, feat_w)
                # visual_hidden_states = hidden_states[:,image_token_start:image_token_start + visual_token_num,:]
                # viusal_sink_hs = visual_hidden_states[:, :1]
                # sim2sink_map = torch.cosine_similarity(visual_hidden_states, viusal_sink_hs, dim=-1).reshape(-1, feat_h, feat_w)
                (sub_img_feats, sub_img_nums, bbox_image_grid_thw,
                 roi_mask, surrounding_bbox) = get_batched_sub_images(pred_roi, src_images, image_processor,
                                                                    encode_image_fn, image_grid_thw, self.roi_conf_thresh, self.is_debug)
                #sub_img_feats = None
                if sub_img_feats is not None:
                    ...
                    hidden_states, attention_mask, valid_token_mask, input_ids_updated = insert_sub_feat(
                        hidden_states, sub_img_feats, sub_img_nums, visual_token_num, image_token_start, #sys tokens
                        labels, attention_mask, roi_mask, temp_input_id)
                    if image_grid_thw is not None:
                        combined_img_grid_thw = torch.cat((image_grid_thw, bbox_image_grid_thw), dim=0)
                    else:
                        combined_img_grid_thw = bbox_image_grid_thw
                    position_ids, rope_deltas = pos_id_fn(
                        input_ids_updated,
                        combined_img_grid_thw,
                        None,
                        None,
                        attention_mask,
                    )
                    if visual_token_num>1024 and question_roi_binaries is not None:
                        question_roi_binaries = question_roi_binaries.flatten(-2)
                        valid_token_mask[:,image_token_start:image_token_start + visual_token_num] = valid_token_mask[:,image_token_start:image_token_start + visual_token_num] & question_roi_binaries

                    position_ids = position_ids[:,:,valid_token_mask[0]]
                    hidden_states = hidden_states[:,valid_token_mask[0],:]
                    attention_mask = attention_mask[:,valid_token_mask[0]]
                    rope_deltas = rope_deltas + input_ids_updated.shape[1] - valid_token_mask.sum()
                    self.rope_deltas = rope_deltas
                    self.generation_attention_mask = attention_mask
                    self.last_pos_id = position_ids[:,:,-1:]
                ...
                if self.two_stage_vanilla:
                    ...
                    ##TODO: update casual mask, cache_position
                    cache_position = torch.arange(
                        0, 0 + hidden_states.shape[1], device=hidden_states.device
                    )
                    causal_mask = self._update_causal_mask(
                        attention_mask, hidden_states, cache_position, None, output_attentions
                    )
                    position_embeddings = self.rotary_emb(hidden_states, position_ids)
                    for decoder_layer in self.layers:
                        ...
                        if len(past_key_values) == self.twig_K and self.twig_T>1:
                            cos, sin = decoder_layer.self_attn.rotary_emb(tmp_values[0], position_ids)
            ...
            for decoder_layer in self.layers:
                ...
                if self.enable_twig and self.roi_enable2stage:
                    if seq_length> 1: #and (visual_token_num==0 or visual_token_num>=577) and image_number is 1
                        for i in range(self.twig_K, len(self.layers)):
                            self.layers[i].self_attn.layer_idx = i #+ self.twig_T - 1
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.enable_twig = False
        if getattr(config, "enable_twig", False):
            # self.processor = AutoProcessor.from_pretrained(model_path, trust_remote_code=True, padding_side='left', use_fast=True)
            self.enable_twig = True
            self.twig_head = copy.deepcopy(self.lm_head)
            self.roi_branch = config.roi_branch if hasattr(config, "roi_branch") else False
            self.roi_source = config.roi_source if hasattr(config, "roi_source") else "qk"
            self.roi_loss = config.roi_loss if hasattr(config, "roi_loss") else 'bce'
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                if n_image_tokens !=0: image_token_start = torch.where(input_ids[0] == self.config.image_token_id)[0][0].item()
        ...
        outputs = self.model(
            ...
            pos_id_fn = self.get_rope_index,
            temp_input_id = input_ids,
        )
        ...
        if labels is not None:
            ...
            if (not self.enable_twig) or self.roi_enable2stage or visual_token_num == 0:
                ...
                hidden_states = self.model.twig_layers[-1].input_layernorm(hidden_states)
                mask_score_samples = []
                roi_target_maps = []
                for i in range(bsz):
                    visual_masks_sample = visual_masks[i]
                    if not visual_masks_sample.any():
                        continue

                    hidden_states_sample = hidden_states[i]
                    visual_tokens_sample = hidden_states_sample[visual_masks_sample].unsqueeze(0)

                    if self.roi_super_type == 'v1':
                        assert roi_target_map is not None, "roi_target_map must be provided for roi branch"
                        # Slice the original tensors to keep the batch dimension for the helper function
                        #query_hidden_states_sample = get_singleturn_query_text_hs(hidden_states[i:i + 1], labels[i:i + 1])
                    elif self.roi_super_type == 'lazy':
                        raise NotImplementedError("Lazy super type is not implemented yet")

                    if self.roi_source == "qk":
                        q, k = self.model.twig_layers[-1].self_attn.q_proj(hidden_states_sample.unsqueeze(0)), self.model.twig_layers[-1].self_attn.k_proj(hidden_states_sample.unsqueeze(0))

                        h_patch, w_patch = image_grid_thw[i][1], image_grid_thw[i][2]
                        query_states, key_states = q, k
                        sample_bsz, q_len, _ = query_states.size()  # sample_bsz will be 1
                        k_len = key_states.size(1)

                        num_heads, head_dim = self.model.twig_layers[-1].self_attn.num_heads, self.model.twig_layers[-1].self_attn.head_dim
                        num_key_value_heads = self.model.twig_layers[-1].self_attn.num_key_value_heads
                        query_states = query_states.view(sample_bsz, q_len, num_heads, head_dim).transpose(1, 2)
                        key_states = key_states.view(sample_bsz, k_len, num_key_value_heads, head_dim).transpose(1, 2)
                        position_embeddings = self.model.rotary_emb(hidden_states[i:i + 1], position_ids[:, i:i + 1])
                        cos, sin = position_embeddings
                        query_states, key_states = apply_multimodal_rotary_pos_emb(
                            query_states, key_states, cos, sin, self.model.twig_layers[-1].self_attn.rope_scaling["mrope_section"]
                        )
                        key_states = repeat_kv(key_states, self.model.twig_layers[-1].self_attn.num_key_value_groups)
                        query_states, query_indices = get_singleturn_query_text_hs_mheads(query_states, labels[i:i+1])
                        key_states = key_states[:,:,visual_masks_sample]
                        if self.roi_multi_head == False:
                            #mask_score = q @ k.transpose(-1, -2)
                        ...
                        target_sample = roi_target_map[i].to(mask_score.device).float() #h_tar, w_tar
                        ...
                        if self.roi_loss == 'mse':
                            ...
                            if target_sum > 0: target_sample = (target_sample / target_sum) * 100

                        mask_score_samples.append(mask_score.flatten())
                        roi_target_maps.append(target_sample.flatten())


                roi_target_map = torch.cat(roi_target_maps, dim=0) if roi_target_maps else torch.tensor([], device=hidden_states.device, dtype=labels.dtype)
                mask_scores = torch.cat(mask_score_samples, dim=0) if mask_score_samples else torch.tensor([], device=hidden_states.device, dtype=torch.float32)

                # Flatten the tokens
                flat_scores = mask_scores.contiguous().view(-1)  # Shape: (visual_token_nums)
                ...
                if scores_for_loss.numel() > 0:  # Check if there are any valid elements to compute loss on
                    if self.roi_loss == 'bce':
                        ...
                        labels_for_loss = flat_original_labels.to(dtype=scores_for_loss.dtype, device=scores_for_loss.device)
            ...
            if hasattr(self.model, 'rope_deltas'): self.rope_deltas = self.model.rope_deltas

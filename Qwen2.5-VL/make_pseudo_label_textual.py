...
def worker_process(chunk_questions, gpu_id, args):
    ...
    for line in pbar:
        ...
        if "Qwen2.5-VL-3B" in args.model_path:
            required_heads = {20: [1, 4], 22: [14]}
            sink_heads = {}
        elif "Qwen2.5-VL-7B" in args.model_path:
            required_heads = {22: [1, 4], 23: [6, 11]}
            sink_heads = {}
        else:
        ...
        if total_grounding_heads > 0:
            for layer, heads in required_heads.items():
                attn_weights = attn_weights_map[f'layer{layer}']['output2images'].reshape(-1, output_shape[0],
                                                                                          output_shape[1])
        ...
        tmp_result = {"question_id": idx, "prompt": cur_prompt, "text": output_text[0], "image": image_file,
